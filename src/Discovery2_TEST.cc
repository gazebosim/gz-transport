...
/// within the tests.
template<typename T> class DiscoveryDerived : public transport::Discovery2<T>
{
  // Documentation inherited.
  public: DiscoveryDerived(const std::string &_pUuid, bool _verbose = false)
    : transport::Discovery2<T>(_pUuid, g_msgPort, _verbose)
  {
  }
...
TEST(DiscoveryTest, TestBasicAPI)
{
  ...
  Discovery2<MessagePublisher> discovery(g_pUuid1, g_msgPort);
...
{
  Discovery2<ServicePublisher> discovery(g_pUuid1, g_srvPort);
...
TEST(DiscoveryTest, TestAdvertise)
{
  ...
  transport::Discovery2<MessagePublisher> discovery1(g_pUuid1, g_msgPort);
  transport::Discovery2<MessagePublisher> discovery2(g_pUuid2, g_msgPort, true);
...
TEST(DiscoveryTest, TestAdvertiseSameProc)
{
  ...
  // Create two discovery nodes simulating they are in different processes.
  transport::Discovery2<MessagePublisher> discovery1(g_pUuid1, g_msgPort);
  transport::Discovery2<MessagePublisher> discovery2(g_pUuid1, g_msgPort);
...
TEST(DiscoveryTest, TestDiscover)
{
  ...
  Discovery2<MessagePublisher> discovery1(g_pUuid1, g_msgPort);
  discovery1.Start();
  ...
  Discovery2<MessagePublisher> discovery2(g_pUuid2, g_msgPort);
  discovery2.Start();
...
TEST(DiscoveryTest, TestNodeBye)
{
  ...
  // Create two discovery nodes.
  std::unique_ptr<Discovery2<MessagePublisher>> discovery1(
    new Discovery2<MessagePublisher>(g_pUuid1, g_msgPort));
  Discovery2<MessagePublisher> discovery2(g_pUuid2, g_msgPort);
