...
TEST(RepStorageTest, RepStorageAPI)
{
  ...
  transport::IRepHandler_M m;
  transport::RepStorage reps;
  ...
  EXPECT_FALSE(reps.HasHandlerForTopic(topic));
  reps.RemoveHandler(topic, nUuid1);
  EXPECT_FALSE(reps.HasHandlerForNode(topic, nUuid1));
  ...
  EXPECT_TRUE(reps.HasHandlerForTopic(topic));
  EXPECT_TRUE(reps.HasHandlerForNode(topic, nUuid1));
  EXPECT_FALSE(reps.HasHandlerForNode(topic, nUuid2));
  ...
  EXPECT_TRUE(reps.HasHandlerForTopic(topic));
  EXPECT_TRUE(reps.HasHandlerForNode(topic, nUuid1));
  EXPECT_TRUE(reps.HasHandlerForNode(topic, nUuid2));
  EXPECT_TRUE(reps.GetHandlers(topic, m));
  EXPECT_EQ(m.size(), 2);
  ...
  reps.RemoveHandler(topic, nUuid2);
  EXPECT_TRUE(reps.HasHandlerForTopic(topic));
  EXPECT_TRUE(reps.HasHandlerForNode(topic, nUuid1));
  EXPECT_FALSE(reps.HasHandlerForNode(topic, nUuid2));
  EXPECT_TRUE(reps.GetHandlers(topic, m));
