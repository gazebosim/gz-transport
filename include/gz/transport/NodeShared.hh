...
namespace gz::transport
{
  ...
  class GZ_TRANSPORT_VISIBLE NodeShared
  {
    ...
    /// centralized topic subscribers, liveliness subscribers) and then
    /// closes the session explicitly. Closing in this order avoids the
    /// at-exit race where the session destructor would otherwise wait
    /// for callbacks on entities that are still being undeclared.
