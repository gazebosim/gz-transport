...
#pragma warning(pop)
    ...
    /// \class ConnectionMsg Packet.hh ignition/transport/Packet.hh
    /// \brief Connection packet used in the discovery protocol for notifying
    /// a publisher or service provider that we're connecting.
    class IGNITION_TRANSPORT_VISIBLE ConnectionMsg
    {
      /// \brief Constructor.
      public: ConnectionMsg() = default;
      ...
      /// \brief Get the node UUID of the message.
      /// \return Node UUID.
      /// \sa SetNUuid.
      ...
      /// \brief Set the node UUID of the message.
      ...
      /// \brief Set the message type of this message.
      /// \param[in] _typeName New message type.
      /// \sa TypeName.
      public: void SetTypeName(const std::string &_typeName);
      ...
      /// \brief Stream insertion operator.
      /// \param[out] _out The output stream.
      /// \param[in] _msg ConnectionMsg message to write to the stream.
      public: friend std::ostream &operator<<(std::ostream &_out,
                                              const ConnectionMsg &_msg)
      {
        _out << _msg.Header()
             << "Body:" << std::endl
             << "\tNode UUID: [" << _msg.NUuid()    << "]" << std::endl
             << "\tTopic: ["     << _msg.Topic()    << "]" << std::endl
             << "\tType: ["      << _msg.TypeName() << "]" << std::endl;
