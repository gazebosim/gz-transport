...
namespace ignition
{
  namespace transport
  {
    ...
    /// \def IReqHandler_M
    /// \brief Map to store the different service request handlers for a topic.
    /// Each node can have its own request handler. The node id is used as key.
    /// The value is another map, where the key is the request UUID and the
    /// value is pointer to a generic request handler.
    using IReqHandler_M =
    ...
    /// callback contains the publisher's information.
