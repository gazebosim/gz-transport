...
Medium level of debugging support
   This can be obtained installing the ``libignition-transport3-dbg`` package or compiling Ignition Transport from source using the ``RELWITHDEBINFO`` ``CMAKE_BUILD_TYPE`` mode (which is the default if no mode is provided). This will set up ``-O2`` optimization level but provide debugging symbols. This should be the default when firing up gdb to explore errors and submit traces.
