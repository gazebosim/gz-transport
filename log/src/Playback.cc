    ...
    [this, batch{std::move(_batch)}] () mutable
    {
      ...
      for (const Message &msg : batch)
      {
        ...
        if (publishedFirstMessage)
        {
          ...
          auto FinishedWaiting = [this, &startTime, &target, &totalTimePaused, &now]() -> bool
          {
          ...
          if(this->paused){
            ...
            const std::chrono::nanoseconds startTimePaused(std::chrono::steady_clock::now().time_since_epoch());
            // If paused, the thread will be blocked here.
            pauseConditionVariable.wait(lk);
            // Calculate how much time has been paused.
            const std::chrono::nanoseconds elapsedTimePaused(std::chrono::steady_clock::now().time_since_epoch() - startTimePaused);
...
void PlaybackHandle::Implementation::Resume()
{
  ...
  if(this->paused) {
    this->paused = false;
    this->pauseConditionVariable.notify_one();
  }
}
